...
class Logar
{
    ...
    $query ="SELECT pessoa.idpessoa, pessoa.email,pessoa.senha FROM pessoa JOIN funcionario on pessoa.idpessoa=funcionario.idpessoa where pessoa.email=?";
    $prepare=$con->pdo->prepare($query);
    if($prepare->execute(array($email))){
        if($prepare->rowCount()>0){
          $f=$prepare->fetch(PDO::FETCH_ASSOC);
            if(password_verify($senha,$f["senha"])){
                ...
                $_SESSION['id_funcionario']=$f['idpessoa'];
                return true;
            }  
        }
    }
